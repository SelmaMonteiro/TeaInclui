function getPrefs() {
  try { return JSON.parse(localStorage.getItem('tea_prefs') || '{}'); } catch { return {}; }
}
...
function applyPrefs() {
  ...
  // Mostrar/ocultar overlay de exercício guiado em qualquer página
  if (p.calm) {
    ensureCalmOverlay();
  } else {
    destroyCalmOverlay();
...
// --- Exercício de respiração (overlay) ---
let calmTimer = null;
...
function startCalmCycle() {
  ...
  if (reduce) {
    // Sem animação, apenas instruções em tempo real
    runPhase(textEl);
  } else {
    runPhase(textEl);
  }
